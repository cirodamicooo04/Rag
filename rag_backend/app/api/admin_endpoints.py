...
async def upload_and_process_document(background_tasks: BackgroundTasks ,file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    # Salvo il file in uno store temporaneo usando un UUID per evitare QUALSIASI collisione di nomi
    ...
    file_hash = sha256_file(temp_uuid_path)
...
@router.get("/docs")
async def get_document_status(hashes: list[str] | None = Query(None), db: Session = Depends(get_db)):
    docs = crud_docs.get_all_documents(db, hashes)
